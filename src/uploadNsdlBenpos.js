...
// DD-MMM-YY (e.g. "28-AUG-26") → ISO 8601 UTC string, or null if blank/invalid.
// 2-digit year: add 2000 (26 → 2026).
function parseDDMMMYY(raw) {
  ...
  if (!Number.isFinite(day) || !Number.isFinite(yyNum)) return null;
  return new Date(Date.UTC(2000 + yyNum, monthIdx, day)).toISOString();
}

// ── Field type sets ───────────────────────────────────────────────────────────

const DATE_FIELDS = new Set(['date', 'minorDateOfBirth', 'rbiApprovalDate']);
...
const NUMERIC_FIELDS = new Set([
  ...
  'beneficiaryRematPositions', 'beneficiaryRematLockInPositions', 'beneficiaryCmIddPositions',
  'cmPoolPositions', 'ccSettlementPositions', 'beneficiaryFreePositionsHoldNdu',
  'beneficiaryLockInPositionsHoldNdu', 'beneficiaryUnconfirmedFreePositionsHoldNdu',
  'beneficiaryUnconfirmedLockInPositionsHoldNdu',
]);
...
async function pollUntilDone(jobId, batchNum, totalBatches, batchSize, recordsBefore, totalRecords, onProgress) {
  while (true) {
    ...
    const pollRes  = await api.get(`/admin/v1/benpos-nsdl/bulk/${jobId}`);
...
/*
 * uploadNsdlBenpos — converts, chunks, posts and polls the full parsed dataset.
 *
 * onProgress shape matches uploadCdslBenpos — see that module for docs.
 * Returns { totalSucceeded, totalFailed, errors[] }.
 */
export async function uploadNsdlBenpos(records, onProgress) {
  const chunks = [];
  ...
  for (let batchIdx = 0; batchIdx < chunks.length; batchIdx++) {
    ...
    const res     = await api.post('/admin/v1/benpos-nsdl/bulk', payload);
